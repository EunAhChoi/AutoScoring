...
def transform(img, empty):
    ...
    empty = cv2.resize(empty, (960,960), interpolation=cv2.INTER_AREA)
    ...
    ret,thresh = cv2.threshold(imgray,143,255,cv2.THRESH_BINARY)
    ...
    if abs(point1_X - point2_X) > 50 and abs(point1_Y - point2_Y) > 50:    
        ...
        h, w = img.shape[:2] 
        pts1 = np.float32([bottom, left, right, top]) 
        pts2 = np.float32([[0, 0], [w, 0], [0, h], [w, h]]) 
        M = cv2.getPerspectiveTransform(pts1, pts2) 
        img2 = cv2.warpPerspective(img, M, (w, h)) 
        #cv2.circle(img, (top[0], top[1]), 20, (255, 0, 0), -1) 
        ...
        return_list.append(img2)

        # 역행렬 연산
        N = linalg.inv(M)
        print(N)

        tempEmpty = cv2.warpPerspective(empty, N, (960, 960))
        
        #plt.subplot(1, 2, 1), plt.imshow(empty), plt.title('Empty') 
        #plt.subplot(1, 2, 2), plt.imshow(tempEmpty), plt.title('Inverse Empty')
        #plt.show()
        ...
        #img = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)    
        imgray = cv2.cvtColor(tempEmpty,cv2.COLOR_BGR2GRAY)
